...
> {
  ...
  delayedDragEnterLogic: Map<Key, number>;

  loadingRetryTimes: Map<Key, number> = new Map();
  ...
  onNodeDragEnter = (event: React.DragEvent<HTMLDivElement>, node: NodeInstance<TreeDataType>) => {
    ...
    if (!this.delayedDragEnterLogic) {
      this.delayedDragEnterLogic = new Map();
    }
      ...
      this.setState(({ loadedKeys = [], loadingKeys = [] }): any => {
        ...
        promise
          ...
          .catch(e => {
            ...
            this.loadingRetryTimes.set(key, (this.loadingRetryTimes.get(key) || 0) + 1);
            if (this.loadingRetryTimes.get(key) >= MAX_RETRY_TIMES) {
              const { loadedKeys: currentLoadedKeys } = this.state;
