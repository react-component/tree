...
const InternalTree = <TreeDataType extends DataNode | BasicDataNode = DataNode>(
  ...
  const delayedDragEnterLogicRef = React.useRef<Record<SafeKey, number>>({});
  ...
  const clearDelayedDragEnterLogic = React.useCallback(() => {
    Object.values(delayedDragEnterLogicRef.current).forEach(timeoutId => {
  ...
  const scrollTo = React.useCallback<ScrollTo>(scroll => {
    if (scroll && typeof scroll === 'object' && scroll.autoExpand) {
      setExpandedKeys(arrAdd(stateRef.current.expandedKeys, scroll.key));
    }
  ...
  }, [clearDelayedDragEnterLogic, onGlobalMouseUp, onWindowDragEnd]);
