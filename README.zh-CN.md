...
| checkable | 是否支持勾选 | bool/React Node | false |
...
| filterTreeNode | 根据需要过滤一些树节点。它应该返回 true | function(node) | - |
...
| onCheck | 单击树节点/复选框来触发 | function(checkedKeys, e:{checked: bool, checkedNodes, node, event, nativeEvent}) | - |
| onExpand | fire on treeNode expand or not | function(expandedKeys, {expanded: bool, node, nativeEvent}) | - |
| onDragEnd | 当触发树的 Dragend 事件时执行 | function({event,node}) | - |
| onDragEnter | 当触发树的 Dragenter 事件时执行 | function({event,node,expandedKeys}) | - |
| onDragLeave | 当触发树的 Dragleave 事件时执行 | function({event,node}) | - |
| onDragOver | 当触发树的 Dragover 事件时执行 | function({event,node}) | - |
| onDragStart | 当触发树的 Dragstart 事件时执行 | function({event,node}) | - |
...
> 注意：如果你有很多TreeNode，比如超过1000个，让父节点默认折叠起来，效果会很明显，速度很快。因为隐藏在 children 中的 TreeNode 不会插入到 DOM 中。

| name | description | type | 默认 |
| --- | --- | --- | --- |
| className | 给treeNode添加一个类 | String | '' |
| checkable | 如果树可检查，则控制节点可检查 | bool | false |
| style | 将样式设置为treeNode | Object | '' |
